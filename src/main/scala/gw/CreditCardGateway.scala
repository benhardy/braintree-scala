...
class CreditCardGateway(http: Http, configuration: Configuration) {
   ...
   *         { @link net.bhardy.braintree.scala.exceptions.NotFoundException}.
   */
  ...
  private[braintree] def fetchExpiredCreditCards(ids: List[String]): List[CreditCard] = {
  ...
  private[braintree] def fetchExpiringCreditCards(ids: List[String], queryString: String): List[CreditCard] = {
    val query = new IdsSearchRequest().ids.in(ids)
