...
class SubscriptionSearchRequestSpec extends FunSpec with MustMatchers {
  describe("toXml") {
    it("daysPastDueXmlIsOperator") {
      ...
      val actual = new SubscriptionSearchRequest().daysPastDue.is("42").toXML
    ...
    it("priceXmlBetweenOperator") {
      ...
      val actual = new SubscriptionSearchRequest().price.between(new BigDecimal(5), new BigDecimal(15)).toXML
      actual must be === (expected.toString)
    }
    ...
    it("priceXmlIsOperator") {
      ...
      val actual = new SubscriptionSearchRequest().price.is(new BigDecimal(5)).toXML
      actual must be === (expected.toString)
    }
    ...
    it("toXMLEscapesXmlOnMultipleValueNodes") {
