    ...
    onGatewayIt("returnsAllPlans") {
      gateway =>
        ...
        val request = new PlanRequest().billingDayOfMonth(1).billingFrequency(1).currencyIsoCode("USD").
          ...
          price(BigDecimal("100.00")).trialDuration(1).trialDurationUnit(Plan.DurationUnit.DAY).trialPeriod(false)
        http.post("/plans/create_plan_for_tests", request)

        val addOnRequest = new FakeModificationRequest().amount(BigDecimal("100.00")).
        ...
        val discountRequest = new FakeModificationRequest().amount(BigDecimal("100.00")).
        ...
        inside(plans.filter(_.id == planId)) { case actualPlan :: _ =>
          ...
          actualPlan.price must be === BigDecimal("100.00")
          ...
          addOn.amount must be === BigDecimal("100.00")
          addOn.kind must be === "add_on"

          val discount = actualPlan.discounts.head
          discount.amount must be === BigDecimal("100.00")
