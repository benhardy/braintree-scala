...
class CreditCardRequestSpec extends FunSpec with MustMatchers {
  describe("toXml") {
    it("escapes xml chars") {
      ...
      request.toXml.get.toString must be === "<creditCard><cardholderName>Special Xml Chars &lt;&gt;&amp;&quot;'</cardholderName></creditCard>"
    }

    it("includes bundle") {
      val request = new CreditCardRequest().deviceData("{\"deviceSessionId\": \"dsid_abc123\"")
      request.toXml.toString must include ("dsid_abc123")
    }
